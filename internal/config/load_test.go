...
func TestConfigureProvidersChatGPTOAuthUsesOnlyAccountModels(t *testing.T) {
	...
	discover := false
	cfg := &Config{
		Providers: testMap(map[string]ProviderConfig{
			"openai": {
				APIKey:             "oauth-access",
	...
	cfg.setDefaults("/tmp", "")
	resolverEnv := testEnv(map[string]string{})
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"powershell", "glob", "read", "write"}, coderAgent.AllowedTools)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"powershell", "edit", "write"}, coderAgent.AllowedTools)
...
func TestConfig_configureProviders_ProviderHeaderResolveError(t *testing.T) {
	...
	testEnv := testEnv(map[string]string{
		"OPENAI_API_KEY": "test-key",
		"PATH":           os.Getenv("PATH"),
	})
